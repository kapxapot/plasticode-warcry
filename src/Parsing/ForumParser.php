...
|   Time: Tue, 17 Feb 2004 10:12:32 GMT
|   Release: 1cfb81cb73af596793230ed3ca6120ac
    ...
    function convert($in=array( 'TEXT' => "", 'SMILIES' => 0, 'CODE' => 0, 'SIGNATURE' => 0, 'HTML' => 0)) {
        ...
        if ( $in['CODE'] == 1 ) {
            ...
            $txt = preg_replace_callback(
                "#\[code\](.+?)\[/code\]#is",
                function ($m) {
                    return $this->regex_code_tag($m[1]);
                },
                $txt
            );
            ...
            $txt = preg_replace_callback(
                ...
                function ($m) {
                    return $this->regex_build_url(['html' => $m[2], 'show' => $m[2], 'st' => $m[1]]);
                },
                $txt
            );
            ...
            $txt = preg_replace_callback(
                "#(\[quote(.+?)?\].*\[/quote\])#is",
                function ($m) {
                    return $this->regex_parse_quotes($m[1]);
                },
                $txt
            );
            ...
            if ($in['SIGNATURE'] != 1) {
                ...
                $txt = preg_replace_callback(
                    "#\[html\](.+?)\[/html\]#is",
                    function ($m) {
                        return $this->regex_html_tag($m[1]);
                    },
                    $txt
                );
            ...
            $txt = preg_replace_callback(
                "#(\[flash=)(\S+?)(\,)(\S+?)(\])(\S+?)(\[\/flash\])#i",
                function ($m) {
                    return $this->regex_check_flash($m[2], $m[4], $m[6]);
                },
                $txt
            );
